...
#ifndef MPEG4_SMP_WRITER_H_

#define MPEG4_SMP_WRITER_H_
...
class MPEG4WriterSmp : public MediaWriterSmp {
public:
	//MPEG4WriterSmp();
	enum {
		STATUS_RECORDING = 0,
	...
	int32_t recorder_status;
    MPEG4WriterSmp(const char *filename);
    MPEG4WriterSmp(int fd);
...
#ifdef USE_SECTION_SAVE_FILE
	int64_t getTrackTotalBytes();
#endif
    void		release_ex();  // release in ~MPEG4WriterSmp
    bool   getFilePath(char * filepath);
    int mLowMemoryLimit;
    volatile bool   mDone;                  // Writer thread is done?
#if 1 //change bitrate check buffer data
     void     releaseLastChunk();
     
     void     addCodecSource(const sp<MediaCodecSource> &source);
     static void *bitrateCheckThreadWrapper(void *me);
     
     status_t bitrateCheckThread();
     status_t startBitrateCheckThread();
     void     stopBitrateCheckThread();
     
     sp<MediaCodecSource> mCodecSource;
     bool                 mBitrateCheckDone;
     pthread_t            mBitrateCheckThread;                // Thread id for the writer
     bool                 mBitrateCheckThreadStarted;  // Only writer thread started successfully
     bool 		         mBitrateCheckThreadExit;
     Condition 	         mBitrateCheckThreadExitCondition;
     Condition            mBitrateCheckTimerCondition;
     Mutex                mBitrateCheckLock;
     float                mSdcardSpeed;
     Mutex                mSdcardSpeedLock;
     void                 setSdcardSpeed(float speed);
     float                getSdcardSpeed();
#endif //change bitrate check buffer data

protected:
    virtual ~MPEG4WriterSmp();

    int32_t kTrackSize;
	#ifdef USE_SECTION_SAVE_FILE
	int32_t kAudioTrackSize;
	int32_t kVideoTrackSize;
	#endif
	bool mIsFileRotateRequested;
	volatile bool kinitAudioTrackPrameter;
	volatile bool kinitVideoTrackPrameter;
	volatile bool kCouldReset;
...
private:
	...
	bool mNoWriteMetaData;
    bool isBeforeStarting;
    ...
    bool            mIsFirstChunk;
    bool            mIsFirstAudioChunk;
    volatile bool   mStopByUser;            // should stop this file and start next file?
...
#ifdef USE_SECTION_SAVE_FILE
    Mutex           mAudioRotateDoneLock;
    Condition       mAudioRotateDoneCondition;
    Mutex           mVideoRotateDoneLock;
    Condition       mVideoRotateDoneCondition;
    bool            mAudioRotateDone;
    bool            mVideoRotateDone;

    Mutex           mFirstVideoDoneLock;
    Condition       mFirstVideoDoneCondition;

    bool            mNotifyStop;
    bool            mNotifySdcardDamaged;
	friend class MPEG4WriterEx;
    MPEG4WriterEx *mWriterEx;
#endif
    ...
    void  setBeforeRecordStatus(bool isrecordingstatus);
	...
	bool motionexceedsFileDurationLimit(bool isAudio);
    ...
    MPEG4WriterSmp(const MPEG4WriterSmp &);
    MPEG4WriterSmp &operator=(const MPEG4WriterSmp &);
#ifdef MTK_AOSP_ENHANCEMENT
public:
...
private:
	void		init();
	void		releaseEx();  // release in ~MPEG4WriterSmp
...
#ifdef USE_FILE_CACHE
	friend class MPEG4FileCacheWriterSmp;
	MPEG4FileCacheWriterSmp *mCacheWriter;
