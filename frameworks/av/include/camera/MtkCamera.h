...
enum {
	...
	//cvbs signal
    MTK_CAMERA_MSG_EXT_NOTIFY_CVBS_SIGNAL = 0x00000017,
	//
    // Webcam Hot Plug
    MTK_CAMERA_MSG_EXT_NOTIFY_WEBCAM_HOTPLUG =  0x00000018,

    // AE Lv
    MTK_CAMERA_MSG_EXT_NOTIFY_AE_LV = 0x00000019
};
...
enum {
    ...
    //
    // adas
    MTK_CAMERA_MSG_EXT_DATA_ADAS                = 0x000000014,

	//
	// motion detection
    MTK_CAMERA_MSG_EXT_DATA_MOTION              = 0x000000015,
...
enum {
    ...
    CAMERA_CMD_START_ADAS,
    CAMERA_CMD_STOP_ADAS,
    CAMERA_CMD_START_MOTION_DETECTION,
    CAMERA_CMD_STOP_MOTION_DETECTION,

    //
    CAMERA_CMD_QUERY_AE_LV,
};
...
enum _type {
  TYPE_NONE						=0,
  SOLID_LINE_CROSSING			=1,
  DASHED_LINE_CROSSING			=2
};
...
struct ADASInfo {
	...
	int32_t left_lane_warning;
	int32_t right_lane_waring;
	
	_severity lane_severity;
	_type     lane_type;

	int32_t left_lane_pos[4];
	int32_t right_lane_pos[4];

	int32_t light_value;

	int32_t num_of_vehicles;

	_objectType forward_object;
	_severity   forward_severity;

	int32_t calibration_ready;
