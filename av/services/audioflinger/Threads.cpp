...
#ifdef MTK_AUDIO    //MediaServer NE when boot (especailly in single core or bad performance)
            int count = 0;
            while (binder == 0) {
            binder = 
                defaultServiceManager()->checkService(String16("power"));
            sleep(1);
            ALOGW("[DEBUG] Thread %s cannot connect to the power manager service, %d", mName, ++count);
            }
#endif
        } else {
...
#endif	
#ifdef HAVE_SRSAUDIOEFFECT
//ALOGVV("SRS process mFormat %d, mSinkBufferSize %d, mSampleRate %d, mChannelCount %d", mFormat, mSinkBufferSize, mSampleRate, mChannelCount);
        ...
        if (mBytesRemaining == 0) {
            ...
            if (mWriteMutedData) 
            {
                void *buffer = mEffectBufferValid ? mEffectBuffer : mSinkBuffer;
                    ...
                    audio_format_t format = mEffectBufferValid ? mEffectBufferFormat : mFormat;
                #endif
                    memset(buffer, 0, mNormalFrameCount * mChannelCount * audio_bytes_per_sample(format));
            }
            #endif
...
{
#if defined (ENABLE_STEREO_SPEAKER) || defined (RC_ENABLE_STEREO_SPEAKER) //define in audio_custom_exp.h
...
void AudioFlinger::MixerThread::threadLoop_mix()
{
    ...
    AudioDump::threadDump(fileName.string(), buffer, mNormalFrameCount*mChannelCount *audio_bytes_per_sample(ProcessFormat), gaf_mixer_propty,
        ...
        Vector< sp<Track> > *tracksToRemove)
{
    ...
    int surroundState = (track->streamType() == AUDIO_STREAM_MUSIC)? mAudioFlinger->mBesSurroundState : 0 ;
            ...
            mAudioMixer->setParameter(
                        name,
                        AudioMixer::SURROUND,
                        AudioMixer::BESSURND_ENABLE, (void *) surroundState);
            mAudioMixer->setParameter(
                        name,
                        AudioMixer::SURROUND,
