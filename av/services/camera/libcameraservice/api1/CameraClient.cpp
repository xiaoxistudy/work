...
//!++
    ...
    // b/28466701
    uint8_t *payload =
            reinterpret_cast<uint8_t*>(mem->pointer());
...
//!--
        ...
        return OK;
    } else if(cmd == CAMERA_CMD_START_VIDEO_CALLBACK) {
        enableMsgType(CAMERA_MSG_VIDEO_CALLBACK);
        return OK;
    } else if(cmd == CAMERA_CMD_STOP_VIDEO_CALLBACK) {
        disableMsgType(CAMERA_MSG_VIDEO_CALLBACK);
        return OK;
    }
...
//!++
    // b/28466701
    uint8_t *payload =
            reinterpret_cast<uint8_t*>(dataPtr->pointer());
    if (*(uint32_t*)payload == kMetadataBufferTypeGrallocSource) {
        buffer_handle_t* pBuffer = (buffer_handle_t*)(payload + 4);
        *pBuffer = (buffer_handle_t)((uint8_t*)(*pBuffer) -
                ICameraRecordingProxy::getCommonBaseAddress());
...
int CameraClient::getOrientation(int degrees, bool mirror) {
    LOG1("degrees:%d, mirror:%d", degrees, mirror );
    LOG2("HAL_TRANSFORM_FLIP_H:%d, HAL_TRANSFORM_FLIP_V:%d, HAL_TRANSFORM_ROT_90:%d",
    ...
    if (!mirror) {
        ...
        if (degrees == 0) {           // FLIP_H and ROT_0
            ...
            return HAL_TRANSFORM_ROT_90;        // not mirror.
            //return HAL_TRANSFORM_FLIP_H | HAL_TRANSFORM_ROT_90;
        } else if (degrees == 180) {  // FLIP_H and ROT_180
            return HAL_TRANSFORM_ROT_180;
            //return HAL_TRANSFORM_FLIP_V;
        } else if (degrees == 270) {  // FLIP_H and ROT_270
            return HAL_TRANSFORM_ROT_270;        // not mirror
