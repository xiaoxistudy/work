                                             ...
                                             audio_session_t session)
{
    ...
    if (stream == AUDIO_STREAM_TTS) {
        ...
        if (!isAnyOutputActive(AUDIO_STREAM_TTS)) {
            beaconMuteLatency = 0;
        }
#endif
    }
...
#ifdef MTK_AUDIO
    ALOGV("s %d c %d avaDev %x ba2dpo %d ba2dpsp %d dev %x",strategy,fromCache,availableOutputDeviceTypes,getA2dpOutput(),mA2dpSuspended,device);
#endif
...
#if 0   // Performance issue for GTS RemoteSubmixHostTest
