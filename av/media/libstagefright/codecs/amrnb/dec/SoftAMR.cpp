...
SoftAMR::SoftAMR(
    ...
    : SimpleSoftOMXComponent(name, callbacks, appData, component),
      ...
      mNeedPortReset = true;//for pause and seekto 0
	  mFormatChange = false;//for pause and seekto 0
#endif 
...
#ifdef MTK_AOSP_ENHANCEMENT
		//amr has no csd frame, so do follow things for pause and seekto 0
		if(mNeedPortReset == true)
		{	
				notify(OMX_EventPortSettingsChanged, 1, 0, NULL);
				mOutputPortSettingsChange = AWAITING_DISABLED;
				mNeedPortReset = false;
				mFormatChange = true;				
				return; 
		}
		else
...
#ifdef MTK_AOSP_ENHANCEMENT
	if(mFormatChange == false)//for pause and seekto 0
	{
		mNeedPortReset = true;
	}
#endif
