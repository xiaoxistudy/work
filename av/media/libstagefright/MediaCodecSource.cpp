...
status_t MediaCodecSource::read(
    ...
    *buffer = NULL;
    ...
    if(mOutputBufferQueue.size()>100){
       ALOGW("-- take care Memory Issue! BufCount:%d,mBufferringSize:%lld, mIsVideo:%d",mOutputBufferQueue.size(), mBufferringSize, mIsVideo);
    }
    ...
    if (!mEncoderReachedEOS) {
        *buffer = *mOutputBufferQueue.begin();
        ...
        mBufferringSize -= (*buffer)->size();
        ALOGV("read mIsVideo=%d, buf size:%d, mBufferringSize:%lld .",mIsVideo,(*buffer)->size(),mBufferringSize);
        return OK;
    }
    return mErrorCode;
}
...
#ifdef MTK_AOSP_ENHANCEMENT
#ifdef MTK_SLOW_MOTION_VIDEO_SUPPORT
//when directlink, not push the first config time in mDecodingTimeQueue
...
/*
    if((mAvailEncoderInputIndices.size() == 0)
        && !mIsVideo)
    {
        ALOGD("audio buffer overflow,--Size:%d",mInputBufferQueue.size());
        while (mInputBufferQueue.size() > 100){
            MediaBuffer *mbuf = *mInputBufferQueue.begin();
            mInputBufferQueue.erase(mInputBufferQueue.begin());
            if (mbuf != NULL) {
                mbuf->release();
    ...
    case kWhatEncoderActivity:
    {
        ...
        if (cbID == MediaCodec::CB_INPUT_AVAILABLE) {
            ...
            if (err != OK || outbuf == NULL) {
		   	...
		   	{
			Mutex::Autolock autolock(mOutputBufferLock);
           //zhenwang Modify here, for memory overflow
            if((mBufferringSize > 70*1024*1024)||(!mIsVideo && mOutputBufferQueue.size()>6000)){
				mEncoder->releaseOutputBuffer(index);
				mLastDrop=true;
				ALOGW("Always drop %s, mBufferringSize:%lld",mIsVideo?"Video":"Audio", mBufferringSize);
				break;
            }
			else if(mLastDrop){
			    if((!mIsVideo && mOutputBufferQueue.size()<4000)||
					(mIsVideo&&(mBufferringSize < 50*1024*1024)&&(flags & MediaCodec::BUFFER_FLAG_SYNCFRAME))){
				   mLastDrop = false;
				   ALOGW("Memory Recover, mBufferringSize:%lld", mBufferringSize);
					}
				else{
					mEncoder->releaseOutputBuffer(index);
				    mLastDrop=true;
				    ALOGW("Wait for KeyFrame drop V, mBufferringSize:%lld", mBufferringSize);
					break;
					}
			  }
		   	}
            ...
            mBufferringSize += mbuf->size();
            ALOGV("mIsVideo=%d, buf size:%d, mBufferringSize:%lld",mIsVideo, outbuf->size(), mBufferringSize);
...

            {
                ...
                if(mOutputBufferQueue.size()>100){
                   ALOGW("++ take care Memory Issue! BufCount:%d,mBufferringSize:%lld, mIsVideo:%d",
                          mOutputBufferQueue.size(), mBufferringSize, mIsVideo);
                }
...
status_t MediaCodecSource::setVideoBitRate(int32_t videoBitrate){
    if (mEncoder != NULL)
    {
    	sp<AMessage> param = new AMessage;
    	param->setInt32("video-bitrate", videoBitrate);
    	mEncoder->setParameters(param);
    }
	return OK;
}
status_t MediaCodecSource::setVideoDuration(int64_t videoduration){
    if (mEncoder != NULL)
    {
    	sp<AMessage> param = new AMessage;
    	param->setInt64("video-duration", videoduration);
    	ALOGE("setVideoDuration %lld",videoduration);
    	mEncoder->setParameters(param);
    }
	return OK;
}

int64_t MediaCodecSource::getBufferSize(){
	ALOGE("getBufferSize mBufferringSize:%lld", mBufferringSize);

    return mBufferringSize;
}
