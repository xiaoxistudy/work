...
status_t CameraSource::initWithCameraAccess(
    ...
    memset(param1,0,PROPERTY_VALUE_MAX);
    isEnableLog = 0;
    property_get("enable_camerasource_debug", param1, "0");
    isEnableLog = atol(param1);
	ALOGD("initWithCameraAccess isEnableLog=%d",isEnableLog);
...
void CameraSource::signalBufferReturned(MediaBuffer *buffer) {
    ...
    if(isEnableLog != 0)
	ALOGD("signalBufferReturned: %p,mFramesBeingEncoded.size()=%d", buffer->data(),mFramesBeingEncoded.size());
#else
        ...
        while (mStarted && mFramesReceived.empty()) {
            if (NO_ERROR !=
                ...
                if(mFramesBeingEncoded.size() == mCameraBufferCount){
                	ALOGW("Timed out waiting for encoder return buffer!: %" PRId64 " us", mLastFrameTimestampUs);
                	...
                	ALOGW("Timed out waiting for incoming camera video frames: %" PRId64 " us", mLastFrameTimestampUs);
...
void CameraSource::dataCallbackTimestamp(int64_t timestampUs,
    ...
    if(CAMERA_MSG_VIDEO_CALLBACK == msgType)
    {
        return;
    }
...
void CameraSource::adjustIncomingANWBuffer(IMemory* data) {
    uint8_t *payload =
		...
		if (*(uint32_t*)payload == kMetadataBufferTypeGrallocSource) {
			...
			size_t *pOffset = (size_t *)(payload + sizeof(uint32_t) + sizeof(buffer_handle_t) * 2);
			*pBuffer = (buffer_handle_t)((uint8_t*)(*pBuffer) + *pOffset);
...
void CameraSource::adjustOutgoingANWBuffer(IMemory* data) {
    uint8_t *payload =
	 ...
	 if (*(uint32_t*)payload == kMetadataBufferTypeGrallocSource) {
		...
		size_t *pOffset = (size_t *)(payload + sizeof(uint32_t) + sizeof(buffer_handle_t) * 2);
		*pBuffer = (buffer_handle_t)((uint8_t*)(*pBuffer) - *pOffset);
		//ALOGD("Recoveried adjustOutgoingANWBuffer: [0]=%p [1]=%p [2]=%p", \
							//*pBuffer, *(buffer_handle_t *)(payload + sizeof(uint32_t) + sizeof(buffer_handle_t)), *pOffset);
		}
#else
    if (*(uint32_t*)payload == kMetadataBufferTypeGrallocSource) {
        buffer_handle_t* pBuffer = (buffer_handle_t*)(payload + 4);
