...
{
    int32_t flag;
    if(msg->findInt32("vdec-set-converter", &flag)){
	  
	 OMX_PARAM_CONVERTER Cvertdef;
      Cvertdef.mbCustomConvert= OMX_TRUE;
	  int32_t mConvertWidth=720;
	  int32_t mConvertHeight=480;
	msg->findInt32("mConvertWidth",&mConvertWidth);
	msg->findInt32("mConvertHeight",&mConvertHeight);
	  Cvertdef.mConvertWidth = (OMX_U32)mConvertWidth;
	  Cvertdef.mConvertHeight = (OMX_U32)mConvertHeight;
	  ALOGD("setParameter vdec-set-converter W=%d,H=%d",Cvertdef.mConvertWidth,Cvertdef.mConvertHeight);
      mOMX->setParameter(mNode, OMX_IndexVendorMtkOmxVdecMDPConverter, &Cvertdef, sizeof(Cvertdef));
    }
...
bool ACodec::BaseState::onOMXEmptyBufferDone(IOMX::buffer_id bufferID) {
    //ALOGI("[%s] onOMXEmptyBufferDone %p",
        ...
        case RESUBMIT_BUFFERS:
        {
            if (buffer != NULL && !mCodec->mPortEOS[kPortIndexInput]) {
                ...
                if (flags & OMX_BUFFERFLAG_CODECCONFIG) {
                    ...
                    //ALOGE("[%s] emptyBuffer %p w/ time %lld us",
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
                        ...
                        strlen("OMX.MTK.VIDEO.ENCODER.AVC")))
            {
                ...
                //ALOGI("give LatencyToken %d, %d", (int)ticks, rangeLength);
            }
#ifdef MTK_CLEARMOTION_SUPPORT
...
status_t ACodec::setParameters(const sp<AMessage> &params) {
    ...
    if (params->findInt64("video-duration", &videoDuration)) 
	{
	    status_t err = mOMX->setConfig(
	            mNode,
	            OMX_IndexVendorMtkOmxVencSetDuration,
	            &videoDuration,
	            sizeof(videoDuration));
	    if (err != OK) {
	        ALOGE("setConfig(OMX_IndexConfigVideoBitrate, %d) failed w/ err %d",
	               videoDuration, err);
	        return err;
