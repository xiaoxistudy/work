...
#ifdef MTK_AOSP_ENHANCEMENT
	if(mMuteAudioFlag){
       memset((uint8_t *) buffer->data(),0, bufferSize);
	}else
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT
ALOGV("dataCallback,receive one audio buffer (size =%d,timeUs=%" PRId64 ")",bufferSize,timeUs);
#endif
    return OK;
}
...
ALOGV("queueInputBuffer_l,containing %d frams in this buffer,mPrevSampleTimeUs( %" PRId64 " ),receiving drift timeUs( %" PRId64 " ),new calculated timestampUs( %" PRId64 " ),",\
...
void AudioSource::setMuteAudio(bool bMuteAudio){
   //ALOGD("setMuteAudio, bMuteAudio:%d", setMuteAudio);
   mMuteAudioFlag = bMuteAudio;
}
#endif
