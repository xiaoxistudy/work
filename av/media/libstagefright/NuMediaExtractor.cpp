...
status_t NuMediaExtractor::setDataSource(
    ...
    ALOGW("setDataSource path:%s", path);
    if (mImpl != NULL) {
        ALOGW("setDataSource mImpl exist!");
        //return -EINVAL;
    }
    ...
    if (dataSource == NULL) {
        ALOGW("setDataSource err:%d", __LINE__);
        return -ENOENT;
    }
...
status_t NuMediaExtractor::getTrackFormat(
    ...
    *format = NULL;
	...
	meta->setData(
        ...
        ALOGI("convert vos to esds, size:%d, VOS size:%d", esds->size(), size);
	...
	if (esdsMeta->findData(kKeyESDS, &type, &data, &size)) {
	    ALOGI("convert CodecConfigInfo to ESDS size:%d, profile:%d, sample index:%d, channel:%d", size, profile, sampling_freq_index, channel_configuration);
...
#ifdef MTK_AOSP_ENHANCEMENT
		ALOGW("read on track %d err %d", info->mTrackIndex, err);
#endif
                info->mSampleTimeUs = -1ll;
                continue;
            } else {
...
#ifdef MTK_AOSP_ENHANCEMENT
	ALOGW("buffer size:%d < sampleSize:%d", buffer->capacity(), sampleSize);
#endif
        return -ENOMEM;
    }
...
#ifdef MTK_AOSP_ENHANCEMENT
    XLOGV("track:%d, ts:%lld, size:%d", info->mTrackIndex, info->mSampleTimeUs, sampleSize);
#endif
