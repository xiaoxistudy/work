    ...
    if (mSeeking) {
        ...
        if (pts > 0) {
            ...
            if ((pts - mSeekTimeUs < 50000 && pts - mSeekTimeUs > -50000)
            	...
            	sprintf(param, "%lld", mOffset);
            	ALOGD("feedMore mOffset:%lld", mOffset);
            	int err = property_set("keypoint.file.offset", param);
            	if(err < 0){
                   ALOGD("feedMore, property_set keypoint.file.offset error");
            	}
