...
int process( LVM_INT16     *pIn,
    ...
    if (pContext->auxiliary) {
        ...
        for (int i=0; i < frameCount*2; i++) { //always stereo here
            pContext->OutFrames32[i] = clamp32(pContext->OutFrames32[i]<<8);
        }
#else
        for (int i=0; i < frameCount*2; i++) { //always stereo here
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE   
        for (int i=0; i < frameCount*2; i++) { //always stereo here
            pContext->OutFrames32[i] = clamp32((int64_t)pContext->OutFrames32[i] + (int64_t)pIn[i]);
        }
#else
       for (int i=0; i < frameCount*2; i++) { //always stereo here
        ...
        if ((pContext->leftVolume != pContext->prevLeftVolume ||
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
            //AudioEffect_32bit+,clamp32...>>>            
            for (int i = 0; i < frameCount; i++)
            {
                pContext->OutFrames32[2*i] = clamp32((int64_t)((vl >> 16) * pContext->OutFrames32[2*i]) >> 12);
                pContext->OutFrames32[2*i+1] =clamp32((int64_t)((vr >> 16) * pContext->OutFrames32[2*i+1]) >> 12);
                vl += incl;
                vr += incr;
             }
            //-
#else
            for (int i = 0; i < frameCount; i++){
                OutFrames16[2*i] = clamp16((LVM_INT32)((vl >> 16) * OutFrames16[2*i]) >> 12);
                OutFrames16[2*i+1] = clamp16((LVM_INT32)((vr >> 16) * OutFrames16[2*i+1]) >> 12);
                vl += incl;
                vr += incr;
            }
#endif
            ...
            if (pContext->leftVolume != REVERB_UNIT_VOLUME ||
                ...
                for (int i = 0; i < frameCount; i++)
                {
                    pContext->OutFrames32[2*i] =clamp32( (int64_t)(pContext->leftVolume * pContext->OutFrames32[2*i]) >> 12);
                    pContext->OutFrames32[2*i+1] = clamp32( (int64_t)(pContext->rightVolume * pContext->OutFrames32[2*i]) >> 12);
                }
#else
                for (int i = 0; i < frameCount; i++) {
                    OutFrames16[2*i] =
    ...
    if (pContext->config.outputCfg.accessMode == EFFECT_BUFFER_ACCESS_ACCUMULATE)
    {
        ...
        { //always stereo here
            pOut[i] =clamp32( (int64_t)pOut[i] + (int64_t)pContext->OutFrames32[i]);
        }
    }else{
        //ALOGV("\tBuffer access is WRITE");
        memcpy(pOut, pContext->OutFrames32, frameCount*sizeof(LVM_INT32)*2);
    }
    //-
#else
    // Accumulate if required
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
//AudioEffect_32bit+
    CHECK_ARG(pConfig->inputCfg.format == AUDIO_FORMAT_PCM_32_BIT);
//-
#else
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
    status = process(    (LVM_INT32 *)inBuffer->raw,
                         (LVM_INT32 *)outBuffer->raw,
