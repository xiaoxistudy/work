...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
//AudioEffect_32bit+
        for (int i = 0; i < inBuffer->frameCount*2; i++)
        {
            inBuffer->s16[i]=inBuffer->s32[i]>>16;
            outBuffer->s16[i]=outBuffer->s32[i]>>16;
        }
//-
#endif
        pContext->pBundledContext->NumberEffectsCalled = 0;
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
//AudioEffect_32bit+
        for (int i =outBuffer->frameCount*2-1; i >= 0; i--)
        {
            outBuffer->s32[i] = clamp32(outBuffer->s16[i]<<16);
        }
//-
#endif	
    } else {
...
//AudioEffect_32bit+            
            for (size_t i=0; i < outBuffer->frameCount*2; i++)
                outBuffer->s32[i] = clamp32((int64_t)outBuffer->s32[i] + (int64_t)inBuffer->s32[i]);
//-
#else
            for (size_t i=0; i < outBuffer->frameCount*2; i++){
                outBuffer->s16[i] =
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
//AudioEffect_32bit+
          memcpy(outBuffer->raw, inBuffer->raw, outBuffer->frameCount*sizeof(LVM_INT32)*2);
//-
#else
