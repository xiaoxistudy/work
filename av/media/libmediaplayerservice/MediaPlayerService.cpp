...
status_t MediaPlayerService::enableRemoteDisplay(const char *iface, const uint32_t wfdFlags) {
        ...
        if (iface != NULL) {
            ...
            if (wfdFlags == WifiDisplaySink::FLAG_SIGMA_TEST_MODE) {
                mRemoteDisplay = new RemoteDisplay(NULL /* client*/, iface, (const sp<IGraphicBufferProducer> &)NULL);
            }
            else
        ...
        const KeyedVector<String8, String8> *headers)
{
    ...
    if (strncmp(url, "content://", 10) == 0) {
        ...
        if((playerType == MATV_AUDIO_PLAYER) || (playerType == FM_AUDIO_PLAYER))
        {
            ...
            mStatus = p->setDataSource(httpService,url, headers);
...
status_t MediaPlayerService::Client::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    MM_LOGI("setDataSource fd=%d, offset=%lld, length=%lld", fd, offset, length);
...
#ifdef MTK_AOSP_ENHANCEMENT
	//for mp3 & ape low power
	int mFd = dup(fd);
	sp<DataSource> dataSource = new FileSource(mFd,offset,length);
	String8 tmp;
	bool err = false;
	err = dataSource->fastsniff(mFd,&tmp);
	//if fastsniff return confidence = 0.0
	//re-sniff for audio/mpeg
	if(err==false)
	{
		float confidence;
		sp<AMessage> meta;
		dataSource->sniff(&tmp,&confidence,&meta);
	}

	ALOGD("mimeType = %s,default playerType = %d",tmp.string(),playerType);
...
status_t MediaPlayerService::Client::setNextPlayer(const sp<IMediaPlayer>& player) {
    ...
    if (c != NULL) {
        ...
        MM_LOGI("[%ld]setNextPlayer:[%ld]",mConnId,mNextClient->mConnId);
...
#ifdef MTK_AOSP_ENHANCEMENT
    MM_LOGI("[%d] notify (%p, %d, %d, %d, %x)", client->mConnId, cookie, msg, ext1, ext2, obj);
                                       ...
                                       const sp<IMemoryHeap>& heap, size_t *pSize)
{
    ...
    MM_LOGI("decode(%d, %lld, %lld)", fd, offset, length);
    ...
    //soundpool swt error handling
    sp<AudioCache> cache;

    if (heap->getHeapID() < 0) goto Exit;

    // create the right type of player
    cache = new AudioCache(heap);
    player = MediaPlayerFactory::createPlayer(playerType, cache.get(), cache->notify);
    if (player == NULL) goto Exit;
    if (player->hardwareOutput()) goto Exit;
...
MediaPlayerService::AudioOutput::AudioOutput(int sessionId, int uid, int pid,
    ...
    MM_LOGI("AudioOutput(%d)", sessionId);
        ...
        const audio_offload_info_t *offloadInfo)
{
...

	{
		 ...
		 SXLOGD("delete mRecycledTrack for hdmi multichannel");
    ...
    if (reuse) {
        ...
        if (reuse) {
            ...
            MM_LOGI("chaining to next output");
            close();
            ...
            intptr_t trackId = gAudioTrackCenter.getTrackId((void*)mTrack.get());
            if (trackId) {
                status_t res = gAudioTrackCenter.reset(trackId);
                if (res != OK) ALOGI("Reset trackId: %p fail in Audio Track Center", (void*)trackId);
            }
            gAudioTrackCenter.updateTrackMaps((void*)mTrack.get(), (void*)this);
#endif
            return OK;
        }
    }
...
void MediaPlayerService::AudioOutput::close()
{
    ...
    MM_LOGI("close");
#ifdef MTK_AOSP_ENHANCEMENT
    Mutex::Autolock autoLock(mTearDownLock);
    ALOGI("close mTearDownLock");
#endif
    mTrack.clear();
}
        ...
        const audio_offload_info_t* /*offloadInfo*/)
{
    ...
    MM_LOGI("open(%u, %d, 0x%x, %d, %d)", sampleRate, channelCount, channelMask, format, bufferCount);
        ...
        void* cookie, int msg, int ext1, int ext2, const Parcel* /*obj*/)
{
    ...
    MM_LOGI("notify(%p, %d, %d, %d)", cookie, msg, ext1, ext2);
