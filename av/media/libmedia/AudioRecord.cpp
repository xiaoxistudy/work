...
AudioRecord::AudioRecord(
        ...
        audio_input_flags_t flags,
        const audio_attributes_t* pAttributes)    
     : mActive(false), mStatus(NO_INIT), mSessionId(0),
      mPreviousPriority(ANDROID_PRIORITY_NORMAL),
      mPreviousSchedulingGroup(SP_DEFAULT),
      mProxy(NULL)

{
    ...
    mStatus = set(inputSource, sampleRate, format, channelMask,
            frameCount, cbf, user, notificationFrames,false,sessionId, transferType, flags,
            pAttributes);
    ...
    if (effectpos > -1)
    {
        ...
        else
        {
            ...
            if ((ieffect & iAEC) > 0)
            {
		        ...
		        AudioEffect *pAEC = new AudioEffect(pUUID,NULL,0,NULL,NULL,mSessionId,0);
		        if (pAEC != NULL)
		        {
            ...
            if ((ieffect & iNS) > 0)
            {
                ...
                AudioEffect *pNS = new AudioEffect(pUUID,NULL,0,NULL,NULL,mSessionId,0);
		        if (pNS != NULL)
		        {
            ...
            if ((ieffect & iAGC) > 0)
            {
                ...
                AudioEffect *pAGC= new AudioEffect(pUUID,NULL,0,NULL,NULL,mSessionId,0);
		        if (pAGC != NULL)
		        {
