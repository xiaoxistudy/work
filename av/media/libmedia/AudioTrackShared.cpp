        ...
        struct timespec *elapsed)
{
    ...
    for (;;) {
        ...
        //ALOGD("front(%p), mIsOut %d, avail %d, mFrameCount %d, filled %d", 
        //    cblk->u.mStreaming.mFront, mIsOut, avail, mFrameCount, filled);
        if (avail > 0) {
        ...
        if (!(old & CBLK_FUTEX_WAKE)) {
            ...
            errno = 0;
            //ALOGD("front(%p), mIsOut %d, obtainBuffer() FUTEX_WAIT", cblk->u.mStreaming.mFront, mIsOut);
...
void ClientProxy::binderDied()
{
    ...
    if (!(android_atomic_or(CBLK_INVALID, &cblk->mFlags) & CBLK_INVALID)) {
        ALOGD("front(%p), mIsOut %d, binderDied() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
...
void ClientProxy::interrupt()
{
    ...
    if (!(android_atomic_or(CBLK_INTERRUPT, &cblk->mFlags) & CBLK_INTERRUPT)) {
        //ALOGD("front(%p), mIsOut %d, interrupt() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
        android_atomic_or(CBLK_FUTEX_WAKE, &cblk->mFutex);
        (void) syscall(__NR_futex, &cblk->mFutex, mClientInServer ? FUTEX_WAKE_PRIVATE : FUTEX_WAKE,
                1);
    }
...
status_t AudioTrackClientProxy::waitStreamEndDone(const struct timespec *requested)
{
    ...
    for (;;) {
        ...
        if (!(old & CBLK_FUTEX_WAKE)) {
            errno = 0;
            //ALOGD("front(%p), mIsOut %d, waitStreamEndDone() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
            (void) syscall(__NR_futex, &cblk->mFutex,
                    mClientInServer ? FUTEX_WAIT_PRIVATE : FUTEX_WAIT, old & ~CBLK_FUTEX_WAKE, ts);
            switch (errno) {
            case 0:            // normal wakeup by server, or by binderDied()
    ...
    if (mIsOut) {
        ...
        if (flush != mFlush) {
            ...
            if (true /*front != newFront*/) {
                ...
                if (!(old & CBLK_FUTEX_WAKE)) {
                    //ALOGD("front(%p), mIsOut %d, obtainBuffer() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
                    (void) syscall(__NR_futex, &cblk->mFutex,
...
void ServerProxy::releaseBuffer(Buffer* buffer)
{
    ...
    if (!mIsOut || (mAvailToClient + stepCount >= minimum)) {
        //ALOGD("front(%p), mIsOut(%d), mAvailToClient=%zu stepCount=%zu minimum=%zu, FUTEX_WAKE", 
        ...
        //ALOGD("front(%p), mIsOut(%d), mAvailToClient=%zu stepCount=%zu minimum=%zu", 
        //    cblk->u.mStreaming.mFront, mIsOut, mAvailToClient, stepCount, minimum);
    }
...
bool  AudioTrackServerProxy::setStreamEndDone() {
    ...
    if (!old) {
        //ALOGD("front(%p), mIsOut %d, setStreamEndDone() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
        (void) syscall(__NR_futex, &cblk->mFutex, mClientInServer ? FUTEX_WAKE_PRIVATE : FUTEX_WAKE,
                1);
    }
    return old;
