...
//!--
    //
    if ( mImageHeap != 0 ) {
        uint32_t *addr = (uint32_t*)(mem->getMemory()->base() + mem->offset());
...
void Camera::setListener(const sp<CameraListener>& listener)
{
    ...
    Mutex::Autolock _l(mLock);
    //mListener = listener;
    Vector<sp<CameraListener> >::iterator it, end;
    for (it = mListenerList.begin(); it != mListenerList.end(); ++it) {
        if((*it) == listener) {
            ALOGE("listener is already exist!");
            return;
        }
    }
...
void Camera::dataCallbackTimestamp(nsecs_t timestamp, int32_t msgType, const sp<IMemory>& dataPtr)
{
    ...
    if (proxylistener != NULL && msgType != CAMERA_MSG_VIDEO_CALLBACK) {
